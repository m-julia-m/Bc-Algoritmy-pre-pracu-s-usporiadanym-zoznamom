...
\textbf{Usporiadaný zoznam} (order data structure) definujeme ako dátovú štruktúru, ktorá uchováva lineárne usporiadanú množinu objektov $M$ a môžeme na nej vykonávať operácie:
\begin{enumerate}
    \item \emph{Insert(x, y)}: vkladá do množiny $M$ nový objekt $y$ ako priameho nasledovníka existujúceho objektu $x$ z množiny $M$. Po vykonaní operácie platí:
    \begin{compactitem}
    \item $x < y$
    ...
    \item \emph{Delete(x)}: odstraňuje z množiny $M$ existujúci objekt $x$.
    \item \emph{Order(x, y)}: odpovedá, či $x$ je predchodca objektu $y$ pre existujúce objekty $x$ aj $y$ z množiny $M$.
\end{enumerate}
...
\caption{Insert(x, y) [linked list]}
\end{algorithm}
...
output\nocite{wiki_list-labeling}, but it will insert its arguments into
the bibliography. The \verb"\nocite{*}" command will insert all the
records in the bibliography database file into the bibliography.
Try uncommenting the command
\nocite{*} %% was cpmmented out originally
